...
function Collection(config) {
    ...
    this.find = function (query, filter, order, options) {
        ...
        if (options.path === undefined) {
            ...
            if (options.path.length > 0 && options.path[0] !== '/') {
                options.path = '/' + options.path;
            }
        }
        options.path = util.format('%s%s', config.client.name, options.path);

        var url = tool.createSolrUrl(config.client, options.path, {
            wt: wt
        });
    };
